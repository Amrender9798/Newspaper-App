...
const SalesTable = ({ newspapers, balance, customerId }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.put(
        `https://newspaper-backend-1.onrender.com/customers/balance/${customerId}`,
        {
      ...
      await axios.post(
        "https://newspaper-backend-1.onrender.com/purchases",
        salesData
      );
  ...
  return (
    <>
      {loading ? (
        ...
        <>
          <ToastContainer autoClose={1500}/> {/* Toast container for displaying toasts */}
